...
/**
 * This class is a PID handler with a feed-forward handler and a complementary
 * filter.
 * 
 * To use PID control: set the kp/ki/kd/kf tuning values calculate output based
 * on the setpoint and measured value regularly
 * 
 * for reference: http://en.wikipedia.org/wiki/PID_controller
...
public class PIDHandler
{
    ...
    private final double ks; // multiplicand for adjusting scale of setpoint to
                             // match scale of measured value
    ...
    private double derivative = 0.0; // approximate slope of input.. units in /
                                     // seconds
    private double dt = .001; // amount of time we waited since our previous
                              // measurement
    private double prevTime = 0.0; // the timestamp of our previous measurement
    private double error = 0.0; // the error (difference between setpoint and
                                // measured value)
    private double prevError = 0.0; // the error during our previous measurement
    private double curTime = 0.0; // the current timestamp
    private double output = 0.0; // the output we wish to set after our
                                 // calculation
     ...
     * 
     * @param kp
     *            scalar for proportional component
     * @param ki
     *            scalar for integral component
     * @param kd
     *            scalar for derivative component
     * @param kf
     *            scalar for feed-forward control
     * @param minOutput
     *            indicates the minimum output value acceptable, or null
     * @param maxOutput
     *            indicates the maximum output value acceptable, or null
     */
    ...
    /**
     * This constructor initializes the object and sets constants to affect
     * gain. This defaults to not utilizing a complementary filter to slow
     * ramp-up/ramp-down.
     * 
     * @param kp
     *            scalar for proportional component
     ...
     *            scalar for feed-forward control
     * @param ks
     *            scalar for adjusting scale difference between measured value
     *            and setpoint value
     * @param minOutput
     *            indicates the minimum output value acceptable, or null
     * @param maxOutput
     ...
     *            scalar for feed-forward control
     * @param kO
     *            scalar for complementary filter multiplier
     * @param kN
     *            scalar for complementary filter multiplier
     * @param minOutput
     *            indicates the minimum output value acceptable, or null
     ...
     */
    public PIDHandler(
        double kp, double ki, double kd, double kf, double kO, double kN, Double minOutput, Double maxOutput)
    {
    ...
    /**
     * This constructor initializes the object and sets constants to affect
     * gain. This utilizes a complementary filter to slow ramp-up/ramp-down.
     * 
     * @param kp
     *            scalar for proportional component
     * @param ki
     ...
     */
    public PIDHandler(
        double kp, double ki, double kd, double kf, double ks, double kO, double kN, Double minOutput, Double maxOutput)
    {
     ...
     *            and setpoint value
     * @param kO
     *            scalar for output complementary filter multiplier
     * @param kN
     *            scalar for output complementary filter multiplier
     * @param kEO
     *            scalar for error complementary filter multiplier
     * @param kEN
     *            scalar for error complementary filter multiplier
     * @param minOutput
     *            indicates the minimum output value acceptable, or null
     ...
     */
    public PIDHandler(
        double kp, double ki, double kd, double kf, double ks, double kO, double kN, double kEO, double kEN,
        Double minOutput,
        Double maxOutput)
    {
        this.ki = ki;
    ...
    /**
     * Calculate the desired output value based on the history, setpoint, and
     * measured value. measuredValue should be in the same unit as the setpoint,
     * basically a positive or negative percentage between -1 and 1. This method
     * should be called in a loop and fed feedback data and setpoint changes
     * 
     * @param setpoint
     *            describes the goal value
     * @param measuredValue
    ...
    public double calculatePosition(double setpoint, double measuredValue)
    {
        ...
        // To prevent division by zero and over-aggressive measurement, output
        // updates at a max of 1kHz
     ...
     * 
     * @param setpoint
     *            describes the goal velocity value
     * @param measuredValue
     *            describes the measured value, where the measured value is the
